...
class DistributedLock(object):
    ...
    def initialize(self):
        ...
        try:
            pids = self.peer_list.get_peers().keys()
            if(not pids or (min(pids) >= self.owner.id)):
    ...
    def acquire(self):
        ...
        if self.state == NO_TOKEN:
            ...
            for pid in self.peer_list.get_peers().keys():
                    ...
                    #continue
                try:
                    print("Trying to request token from: " + pid)
                    self.peer_list.get_peers()[pid].request_token(self.time, self.owner.id)
    ...
    def release(self):
        ...
        for pid in self.peer_list.get_peers().keys():
            if self.request[pid] > self.token[pid] and pid != self.owner.id:
                self.state = NO_TOKEN
                self.token[self.owner.id] = self.time
                self.peer_list.get_peers()[pid].obtain_token(self._prepare(self.token))
                break
    ...
    def request_token(self, time, pid):
        ...
        self.peer_list.lock.acquire()
        try:
            self.request[pid] = self.request[pid] if self.request[pid] > time else time

            if (self.state == TOKEN_PRESENT or self.state == TOKEN_HELD) and self.token[pid] < time:
                self.peer_list.get_peers()[pid].obtain_token(self._prepare(self.token))
                self.state = NO_TOKEN
        finally:
            self.peer_list.lock.release()
