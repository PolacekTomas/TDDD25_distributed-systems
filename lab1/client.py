...
class DatabaseProxy(object):
    ...
    def read(self):
        try:
            ...
            data = json.dumps({"method":"read","args":[]})+"\n"
            server_socket.send(data.encode())
            worker = server_socket.makefile(mode="rw")
            read_msg = worker.readline()
            worker.flush()
            server_socket.close()
            output = json.loads(read_msg)
            if 'error' in output:
                raise type(output['error']['name'], (Exception,), dict())(output['error']['args'])
    ...
    def write(self, fortune):
        try:
            server_socket = socket.socket()
            server_socket.connect(self.address)
            data = json.dumps({"method":"write","args":fortune})+"\n"
            server_socket.send(data.encode())
            worker = server_socket.makefile(mode="rw")
            read_msg = worker.readline()
            worker.flush()
